...
const RobertDeVore2022Colors = ( { postType, editPost } ) => {
	...
	const [ color, setColor ] = usePostMeta( 'robert_devore_accentColor' );

        const checkIcon = <Icon icon={ check } size={ 28 } />

	return(
		<PluginPostStatusInfo>
                        <PanelRow className="components-robert-devore-accent-color-row">
                                <ToggleGroupControl
                                        value={ color }
                                        className="components-robert-devore-accent-color"
                                        onChange={ ( newColor ) => {
                                                setColor( newColor );
...
const RobertDeVore2022ColorswithSelect = compose( [
	withSelect( ( select ) => {
		return {
...
] )( RobertDeVore2022Colors );

registerPlugin( 'robert-devore-colors', { render: RobertDeVore2022ColorswithSelect } );
